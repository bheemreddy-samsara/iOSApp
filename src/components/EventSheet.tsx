...
export function EventSheet({
  ...
  const [needsApproval, setNeedsApproval] = useState(requireApproval || event?.approvalState === 'pending');
  ...
  const updateTime = (type: 'start' | 'end') => (e: DateTimePickerEvent, value?: Date) => {
    if (Platform.OS !== 'ios') {
  ...
  const toggleMember = (memberId: string) => {
    setDraft((prev) => {
      ...
      return {
        ...prev,
        members: exists ? prev.members.filter((id) => id !== memberId) : [...prev.members, memberId]
      };
    });
  };

  const save = () => {
    onSubmit({ ...draft, allDay, approvalState: needsApproval ? 'pending' : 'approved' });
  };
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onDismiss} presentationStyle="pageSheet">
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{mode === 'create' ? 'New Event' : 'Edit Event'}</Text>
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={styles.card}>
            ...
            <Pressable accessibilityRole="button" onPress={() => setShowStartPicker(true)} style={styles.timeRow}>
              <Text style={styles.timeLabel}>Starts</Text>
              <Text style={styles.timeValue}>{new Date(draft.start).toLocaleString()}</Text>
            </Pressable>
            <Pressable accessibilityRole="button" onPress={() => setShowEndPicker(true)} style={styles.timeRow}>
              <Text style={styles.timeLabel}>Ends</Text>
              <Text style={styles.timeValue}>{new Date(draft.end).toLocaleString()}</Text>
            </Pressable>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.privacyRow}>
              {['family', 'private', 'busy-only'].map((modeKey) => {
                const label = modeKey === 'family' ? 'Family' : modeKey === 'private' ? 'Private' : 'Busy only';
                return (
                  <Pressable
                    key={modeKey}
                    accessibilityRole="button"
                    accessibilityState={{ selected: draft.privacyMode === modeKey }}
                    onPress={() => setDraft((prev) => ({ ...prev, privacyMode: modeKey as DraftEvent['privacyMode'] }))}
                    style={[styles.privacyChip, draft.privacyMode === modeKey && styles.privacyChipActive]}
                  >
                    <Text style={[styles.privacyLabel, draft.privacyMode === modeKey && styles.privacyLabelActive]}>{label}</Text>
                  </Pressable>
                );
              })}
            </View>
              ...
              {members.map((member) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={[styles.memberName, active && styles.memberNameActive]}>{member.name.split(' ')[0]}</Text>
                  </Pressable>
                );
              })}
            </ScrollView>
            ...
            <View style={styles.card}>
              ...
              <View style={styles.switchRow}>
                ...
                <Switch
                  ...
                  trackColor={{ false: colors.surfaceMuted, true: colors.categoryPink }}
                  thumbColor={needsApproval ? colors.categoryPink : '#f4f3f4'}
                />
              </View>
              <Text style={styles.helper}>Kids submit events for review. Parents get a gentle ping to approve.</Text>
            </View>
          )}

          {mode === 'edit' && onDelete && event?.id && (
            <Pressable accessibilityRole="button" onPress={() => onDelete(event.id!)} style={styles.deleteButton}>
      ...
      {showStartPicker && (
        <DateTimePicker value={new Date(draft.start)} mode="datetime" onChange={updateTime('start')} />
      )}
      {showEndPicker && (
        <DateTimePicker value={new Date(draft.end)} mode="datetime" onChange={updateTime('end')} />
      )}
    </Modal>
  );
}
