...
export const EventCard = memo(({ event, onPress, showStatus = true, compact = false }: EventCardProps) => {
  ...
  const attendees = event.attendees?.map((id) => memberLookup.get(id)).filter(Boolean).slice(0, 3) ?? [];
    ...
    >
      <View style={styles.header}>
        <View>
          ...
          {event.location && !event.isBusyOnly && <Text style={styles.location}>{event.location}</Text>}
        </View>
        {showStatus && <ApprovalPill state={event.approvalState} />}
      </View>
      {!event.isBusyOnly && event.description && !compact && <Text style={styles.description}>{event.description}</Text>}
